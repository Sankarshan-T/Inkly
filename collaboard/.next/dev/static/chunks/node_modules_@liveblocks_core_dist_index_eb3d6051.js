...
function createApiClient({ baseUrl, authManager, currentUserId, fetchPolyfill }) {
    ...
    async function sendMessagesOverHTTP(options) {
        return httpClient.rawPost(url`/v2/c/rooms/${options.roomId}/send-message`, await authManager.getAuthValue({
            requestedScope: "room:read",
            roomId: options.roomId
        }), {
            nonce: options.nonce,
...
var LiveList = class _LiveList extends AbstractCrdt {
   ...
   */ _insertAndSort(item) {
    ...
    #applySetAck(op) {
        ...
        if (existingItem !== void 0) {
            ...
            if (indexOfItemWithSamePosition !== -1) {
                this.#implicitlyDeletedItems.add(this.#items[indexOfItemWithSamePosition]);
   ...
   */ #detachItemAssociatedToSetOperation(deletedId) {
    ...
    #applySetChildKeyRemote(newKey, child) {
        ...
        if (existingItemIndex === -1) {
            ...
            this.#items[existingItemIndex]._setParentLink(this, makePosition(newKey, this.#items[existingItemIndex + 1]?._parentPos));
            const previousIndex = this.#items.indexOf(child);
            child._setParentLink(this, newKey);
            this._sortItems();
            const newIndex = this.#items.indexOf(child);
    ...
    #applySetChildKeyUndoRedo(newKey, child) {
        ...
        if (existingItemIndex !== -1) {
            actualNewKey = makePosition(newKey, this.#items[existingItemIndex + 1]?._parentPos);
        }
   ...
   */ toArray() {
        return this.#items.map((entry)=>liveNodeToLson(entry));
    }
    /**
   ...
   */ map(callback) {
        return this.#items.map((entry, i)=>callback(liveNodeToLson(entry), //                    ^^^^^^^^
   ...
   */ some(predicate) {
    ...
    #shiftItemPosition(index, key) {
        const shiftedPosition = makePosition(key, this.#items.length > index + 1 ? this.#items[index + 1]?._parentPos : void 0);
    ...
    /** @internal */ _toTreeNode(key) {
        return {
            ...
            id: this._id ?? nanoid(),
            key,
            payload: this.#items.map((item, index)=>item.toTreeNode(index.toString()))
        };
    }
    toImmutable() {
...
var LiveObject = class _LiveObject extends AbstractCrdt {
   ...
   */ static detectLargeObjects = false;
    ...
    /** @private Do not use this API directly */ static _fromItems(items, pool) {
...
function createRoom(options, config) {
    ...
    function* chunkMessages(messages) {
        if (messages.length < 2) {
            if (messages[0].type === ClientMsgCode.UPDATE_STORAGE) {
                ...
                throw new Error("Cannot split into chunks smaller than the allowed message size");
            }
        }
            ...
            case "default":
                {
                    ...
                    const didNotify = config.errorEventSource.notify(err);
                    if (!didNotify) {
                        error2("Message is too large for websockets.  Configure largeMessageStrategy option or useErrorListener to handle this.");
                    }
                    return;
                }
            case "split":
            ...
            case "experimental-fallback-to-http":
                {
                    ...
                    void httpClient.sendMessagesOverHTTP({
                        roomId,
                        nonce,
                        messages
                    }).then((resp)=>{
                        ...
                        error2(`Failed to deliver message over HTTP: ${String(err)}`);
                    });
                    return;
                }
        }
    ...
    async function streamStorage() {
        ...
        const items = await httpClient.streamStorage({
            roomId
        });
